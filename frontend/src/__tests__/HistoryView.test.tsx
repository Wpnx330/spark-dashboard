...
function installFetch(routes: {
  ...
  fetchHandler = async (url, init) => {
    if (url.startsWith('/api/history/settings')) {
      ...
      return routes.settings?.() ?? { enabled: false, utc_offset: '0' }
    }
...
describe('HistoryView', () => {
  beforeEach(() => {
    // Default: settings call returns disabled logging, summary returns data.
    installFetch({})
  })

  it('renders the preset buttons and the Custom toggle', async () => {
    render(<HistoryView />)
    // The seven preset labels are rendered via presetRange().label
  ...
  it('switching to Custom reveals the date-range inputs and Go button', async () => {
    render(<HistoryView />)
    // Custom inputs are hidden initially
  ...
  it('renders summary stats (token throughput + peak load) when data is present', async () => {
    ...
    render(<HistoryView />)
    // Wait for the fetch + render cycle.
    await waitFor(() => {
      expect(screen.getByText('4,242')).toBeInTheDocument()
    })
    expect(screen.getByText('7')).toBeInTheDocument()
    // The "Cost Savings" card must NOT exist — pricing scope was stripped.
    expect(screen.queryByText('Cost Savings')).not.toBeInTheDocument()
    expect(screen.queryByText('Cloud Cost Avoided')).not.toBeInTheDocument()
    expect(screen.queryByText('Net Savings')).not.toBeInTheDocument()
    // The cloud-rate / electricity settings labels must NOT exist.
    expect(screen.queryByText('$/1M Prompt Tokens')).not.toBeInTheDocument()
    expect(screen.queryByText('$/kWh Electricity')).not.toBeInTheDocument()
  })
  ...
  it('opening settings shows the logging toggle and UTC offset (but no pricing fields)', async () => {
    installFetch({
      settings: () => ({ enabled: false, utc_offset: '-4' }),
    })
    render(<HistoryView />)
    ...
    // Pricing-related fields are stripped.
    expect(screen.queryByText('$/1M Gen Tokens')).not.toBeInTheDocument()
    expect(screen.queryByText('Look up pricing')).not.toBeInTheDocument()
  })
  ...
  it('clicking a preset re-fetches the summary with a new range', async () => {
    ...
    render(<HistoryView />)
    // Initial fetch (today) happens on mount.
