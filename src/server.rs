...
pub fn create_router(state: Arc<AppState>, enable_history: bool) -> Router {
    ...
    #[cfg(target_os = "linux")]
    let router = if crate::logs::is_log_viewer_enabled() {
        router.route("/ws/logs", get(crate::logs::ws_logs_handler))
    } else {
        router
    };
...
async fn history_settings(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let tp_gpu_count = state
        .history
        .get_setting("tp_gpu_count")
        .await
        .unwrap_or(None)
...
async fn history_settings_update(
    ...
    if let Some(v) = &body.tp_gpu_count {
        let _ = state.history.set_setting("tp_gpu_count", v).await;
    }
...
async fn history_lookup_pricing(Json(body): Json<LookupPricingBody>) -> impl IntoResponse {
    ...
    if let Some(&(prompt, gen)) = partial.first() {
        ...
        Json(serde_json::json!({
            "error": format!("model '{}' not found on OpenRouter", model_id)
        }))
        .into_response()
    }
}
